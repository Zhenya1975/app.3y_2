...
def ktg_data_reading():
    ...
    ktg_by_month_data_df['month_year'] = ktg_by_month_data_df['month'].astype(str) + "_" + ktg_by_month_data_df[
        'year'].astype(str)
...
def maintanance_jobs_df():
    ...
    maintanance_jobs_df = maintanance_jobs_df.astype(
        {'downtime_plan': float, "month_year_sort_index": int, "year": int, "month": int, "day": int, "hour": int})
...
def downtime_by_categiries_data():
    ...
    model_hours_df_2023_groupped = model_hours_df_2023.groupby(['maintanance_name'], as_index=False)[
        ['downtime_status']].sum()
    downtime_by_catagories_2023_data = model_hours_df_2023_groupped.rename(
        columns={'maintanance_name': 'Вид ТОРО', 'downtime_status': 'Простой, час'})
    ...
    model_hours_df_2024_groupped = model_hours_df_2024.groupby(['maintanance_name'], as_index=False)[
        ['downtime_status']].sum()
    downtime_by_catagories_2024_data = model_hours_df_2024_groupped.rename(
        columns={'maintanance_name': 'Вид ТОРО', 'downtime_status': 'Простой, час'})
    ...
    model_hours_df_2025_groupped = model_hours_df_2025.groupby(['maintanance_name'], as_index=False)[
        ['downtime_status']].sum()
    downtime_by_catagories_2025_data = model_hours_df_2025_groupped.rename(
        columns={'maintanance_name': 'Вид ТОРО', 'downtime_status': 'Простой, час'})
...
def job_codes_prep():
    ...
    job_codes_df = maintanance_job_list_general.groupby(
        ['maintanance_category_id', 'maintanance_name', 'maintanance_category_sort_index'], as_index=False)[
        'strategy_id'].sum()
    job_codes_df = job_codes_df.loc[:,
                   ['maintanance_category_id', 'maintanance_name', 'maintanance_category_sort_index']]
