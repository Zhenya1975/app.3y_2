...
def maintanance(theme_selector, checklist_be, checklist_model_eo):
    ...
    model_eo_filter_full_list = \
    func_model_eo_select_data_prep.model_eo_select_data_prep(be_list_for_dataframes_filtering)[1]
    ...
    total_qty_EO_2023 = \
    functions.total_qty_EO(be_list_for_dataframes_filtering, model_eo_filter_list_for_dataframes_filtering)[0]
    total_qty_EO_2024 = \
    functions.total_qty_EO(be_list_for_dataframes_filtering, model_eo_filter_list_for_dataframes_filtering)[1]
    total_qty_EO_2025 = \
    functions.total_qty_EO(be_list_for_dataframes_filtering, model_eo_filter_list_for_dataframes_filtering)[2]
    ...
    ktg_2023_text = func_update_downtime_graph_data.update_downtime_graph_data(be_list_for_dataframes_filtering,
                                                                               model_eo_filter_list_for_dataframes_filtering)[
        0]
    ...
    ktg_2024_text = func_update_downtime_graph_data.update_downtime_graph_data(be_list_for_dataframes_filtering,
                                                                               model_eo_filter_list_for_dataframes_filtering)[
        1]
    ktg_2024_card_text = 'КТГ по году: {}'.format(ktg_2024_text)

    ktg_2025_text = func_update_downtime_graph_data.update_downtime_graph_data(be_list_for_dataframes_filtering,
                                                                               model_eo_filter_list_for_dataframes_filtering)[
        2]
    ...
    fig_man_hours = func_man_hours_data_prep.man_hours_data_pre(theme_selector, be_list_for_dataframes_filtering,
                                                                model_eo_filter_list_for_dataframes_filtering)
...
def funct_p11_table(n_clicks_p11_table):
    ...
    if n_clicks_p11_table:
        return dcc.send_data_frame(df_new.to_excel, "Простой по месяцам.xlsx", index=False,
                                   sheet_name="Простой по месяцам")
...
def funct(n_clicks_ktg_table):
    ...
    if n_clicks_ktg_table:
        return dcc.send_data_frame(df.to_excel, "maintanance_job_list_general.xlsx", index=False,
                                   sheet_name="maintanance_job_list_general")
...
def parse_contents(contents, filename):
    ...
    try:
        if 'csv' in filename:
            ...
            check_column_list = ['maintanance_code_id', 'maintanance_code', 'maintanance_category_id',
                                 'upper_level_tehmesto_code', 'maintanance_name', 'interval_motohours',
                                 'downtime_planned', 'pass_interval', 'source']
            control_value = 1
        ...
        # загружаем список eo - в ответ получаем список для перепроверки даты ввода в эксплуатацию, даты списания, среднесуточной наработки
            ...
            eo_list_data = eo_list_data.loc[:,
                           ['level_1_description', 'Название технического места', 'eo_code', 'eo_description', 'mvz',
                            'level_2_description', 'operation_start_date', 'operation_finish_date',
                            'avearage_day_operation_hours']]
